...
deploy_individual_service() {
    ...
    echo "helm install team-generator ./helm-charts/team-generator"
    echo ""
...
deploy_platform() {
    ...
    echo "helm install forte-platform ./helm-charts/forte-platform"
    echo ""
    echo "# Install for development environment"
    echo "helm install forte-dev ./helm-charts/forte-platform \\"
    echo "  -f ./helm-charts/forte-platform/values-dev.yaml"
    echo ""
    echo "# Install for production environment"
    echo "helm install forte-prod ./helm-charts/forte-platform \\"
    echo "  -f ./helm-charts/forte-platform/values-prod.yaml"
    echo ""
...
environment_deployments() {
    ...
    echo "  --namespace forte-dev \\"
    echo "  --create-namespace"
    echo ""
    echo "# Staging (production-like, testing)"
    echo "helm install forte-staging ./helm-charts/forte-platform \\"
    echo "  -f values-staging.yaml \\"
    echo "  --namespace forte-staging \\"
    echo "  --create-namespace"
    echo ""
    echo "# Production (high availability, monitoring)"
    echo "helm install forte-prod ./helm-charts/forte-platform \\"
    echo "  -f values-prod.yaml \\"
    echo "  --namespace forte-prod \\"
    echo "  --create-namespace"
}
...
updates_rollbacks() {
    ...
    echo "helm upgrade forte-platform ./helm-charts/forte-platform \\"
    ...
    echo "helm rollback forte-platform 1"
    echo ""
    echo "# View revision history"
    echo "helm history forte-platform"
}
...
debugging() {
    ...
    echo "helm install --dry-run --debug forte-platform ./helm-charts/forte-platform"
    echo ""
    echo "# Validate chart syntax"
    echo "helm lint ./helm-charts/forte-platform"
    echo ""
    echo "# Template and save to file"
    echo "helm template forte-platform ./helm-charts/forte-platform > generated-manifests.yaml"
    echo ""
    echo "# Check what will be changed in upgrade"
    echo "helm diff upgrade forte-platform ./helm-charts/forte-platform"
    echo ""
    echo "# Get values for deployed release"
    echo "helm get values forte-platform"
}
...
monitoring() {
    ...
    echo "kubectl get all --all-namespaces -l app.kubernetes.io/part-of=forte-platform"
    echo ""
    echo "# View logs from all services"
    echo "kubectl logs -l app.kubernetes.io/part-of=forte-platform --all-containers=true"
}
...
cleanup() {
    ...
    echo "helm uninstall forte-platform"
    echo ""
    echo "# Uninstall and delete persistent data"
    echo "helm uninstall forte-platform --cascade=foreground"
    echo ""
